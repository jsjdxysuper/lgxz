...
void get_date_time(char* buf1, char* buf2)
{
	time_t rawtime; 
	struct tm* timeinfo;
	...
	time(&rawtime); 
	timeinfo = localtime(&rawtime); 
	sprintf(buf1, "%04d-%02d-%02d", (timeinfo->tm_year+1900), timeinfo->tm_mon + 1, timeinfo->tm_mday);
	sprintf(buf2, "%02d:%02d:%02d", timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec);
}
