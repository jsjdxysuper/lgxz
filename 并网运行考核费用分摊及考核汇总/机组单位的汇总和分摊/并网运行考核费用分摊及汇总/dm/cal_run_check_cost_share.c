...
//================================================================================
	...
	printf("加入非停考核结果和计划曲线考核结果后:工厂的数目%d\n",*plant_num);
	printf("工厂的名称     ,工程考核费用\n");
	for(i=0;i<*plant_num;i++)
	{
		for(j=0;j<COLLECT_NUM;j++)
...
//=========================================================================================
	...
	*/
	...
	printf("加入非停考核结果和计划曲线考核结果和十五项后:工厂的数目%d\n",*plant_num);
	printf("工厂的名称     ,工程考核费用\n");
	for(i=0;i<*plant_num;i++)
	{
		for(j=0;j<COLLECT_NUM;j++)
...
int calculate_run_check_cost_share(char* month)
{
	...
	if(-1==get_unit_info(&cost_share_array,&cost_collect17,month,&plant_num))
		{printf("执行get_unit_info()失败\n");return -1;}
		
	if(-1==get_cost_result(month,&cost_share_array, &cost_collect17, &plant_num))
	...
	if(-1==write_share_stru(month,&cost_share_array, &cost_collect17, &plant_num))
